...
QcColorBand *QcGaugeWidget::addColorBand(float position, QList<QPair<QColor,float>> colorList)
{
    QcColorBand * item = new QcColorBand(this, colorList);
    item->setPosition(position);
    mItems.append(item);
    return item;
}
...
QcColorBand::QcColorBand(QObject *parent, QList<QPair<QColor,float>> colorList) :
    QcScaleItem(parent)
{
    mBandColors.append(colorList[0]);
    mBandColors.append(colorList[1]);
    mBandColors.append(colorList[2]);
